...
const ERC20_CONTRACTS: { [symbol: string]: { address: `0x${string}` | undefined, decimals: number } } = {
    'USDT': { address: process.env.NEXT_PUBLIC_USDT_CONTRACT_ADDRESS as `0x${string}`, decimals: 18 },
    'USDC': { address: process.env.NEXT_PUBLIC_USDC_CONTRACT_ADDRESS as `0x${string}`, decimals: 18 },
    'WETH': { address: process.env.NEXT_PUBLIC_WETH_CONTRACT_ADDRESS as `0x${string}`, decimals: 18 },
    'LINK': { address: process.env.NEXT_PUBLIC_LINK_CONTRACT_ADDRESS as `0x${string}`, decimals: 18 },
    'BNB': { address: process.env.NEXT_PUBLIC_BNB_CONTRACT_ADDRESS as `0x${string}`, decimals: 18 },
    'SOL': { address: process.env.NEXT_PUBLIC_SOL_CONTRACT_ADDRESS as `0x${string}`, decimals: 18 },
};
...
export async function sendTokensAction(
  ...
  if (tokenSymbol === 'ETH') {
    ...
    if (!contractInfo || !contractInfo.address) {
      throw new Error(`Contract for ${tokenSymbol} is not configured in environment variables.`);
    }
